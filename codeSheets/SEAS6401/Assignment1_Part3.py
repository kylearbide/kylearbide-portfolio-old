...
from pyspark.sql.functions import col, lower, translate, count
allLower = (df.withColumn("l-first", lower(col("firstName")))
            .withColumn("l-middle", lower(col("middleName")))
            .withColumn("l-last", lower(col("lastName")))
            .withColumn("joinedSSN", translate(col("ssn"),"-",""))
           )
newDF = (allLower.dropDuplicates(["l-first","l-middle","l-last","joinedSSN"])
         .drop("l-first","l-middle","l-last","joinedSSN")
        )
#this should drop all the duplicates
#newDF.count()
#should be 100000
